...
def push_gift_broadcast(gift_order):
    ...
    if is_crown_gift and crown_template:
        ...
        selected_source = ''
    sent = False
    # 判断是否有用户自定义播报频道（覆盖公共频道）
    boss_ch = (getattr(boss, 'broadcast_channel', None) or '').strip()
    player_ch = (getattr(player, 'broadcast_channel', None) or '').strip()
    user_override_channels = set()
    if boss_ch:
        user_override_channels.add(boss_ch)
    if player_ch:
        user_override_channels.add(player_ch)
...
def push_recharge_broadcast(user, amount):
    ...
    # 用户自定义播报频道 → 完全覆盖公共频道
    user_ch = (getattr(user, 'broadcast_channel', None) or '').strip()
    target_ch = user_ch if user_ch else cfg.channel_id
    _async_send(_send_channel_msg, target_ch, card_json)
